...
import { fetchX402Jobs, x402CycleSteps, x402StatusLabel, type X402Job } from "./x402Jobs";

type X402JobsPanelProps = {
  mandate?: string;
};

export function X402JobsPanel({ mandate }: X402JobsPanelProps) {
  ...
  return (
    <div className="dashboard-card x402-jobs-card">
      ...
      {status === "loading" && <Skeleton className="x402-jobs-skeleton" />}
          ...
          {jobs.map((job) => (
            <article className="x402-job-row" key={job.x402_payment_id}>
              ...
              <div className="x402-job-meta">
                ...
                {job.mandate && <span><b>Mandate</b><code>{shortAddress(job.mandate)}</code></span>}
                {!job.payable && (
